...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-sign-single { background: #17a2b8; color: #fff; }
    .btn-sign-single:hover:not(:disabled) { background: #138496; }
    .btn-sign-fail { background: #dc3545; color: #fff; }
    .btn-sign-fail:hover:not(:disabled) { background: #bd2130; }
    .btn-download-fail { background: #fd7e14; color: #fff; }
    .btn-download-fail:hover:not(:disabled) { background: #e06a0d; }
    ...
    (function () {
      ...
      async function loadPdfList() {
        ...
        try {
          ...
          +   '<button class="btn-action btn-sign-single" data-file="' + escapeAttr(f.filename) + '">' + ICO_SIGN + ' Firmar CSV</button>'
          +   (f.filename === 'manifiesto-1.pdf'
                ? '<button class="btn-action btn-sign-fail" data-file="' + escapeAttr(f.filename) + '">' + ICO_SIGN + ' Subida fallida</button>'
                : '')
          +   (f.filename === 'manifiesto-10.pdf'
                ? '<button class="btn-action btn-download-fail" data-file="' + escapeAttr(f.filename) + '">' + ICO_SIGN + ' Descarga fallida</button>'
                : '')
          ...
          // Listeners Firmar CSV individual
          document.querySelectorAll('.btn-sign-single').forEach(function(btn) {
            btn.addEventListener('click', function() { doSignSingleCSV(btn.getAttribute('data-file')); });
          });

          // Listeners Subida fallida
          document.querySelectorAll('.btn-sign-fail').forEach(function(btn) {
            btn.addEventListener('click', function() { doSignFailCSV(btn.getAttribute('data-file')); });
          });

          // Listeners Descarga fallida
          document.querySelectorAll('.btn-download-fail').forEach(function(btn) {
            btn.addEventListener('click', function() { doDownloadFailCSV(btn.getAttribute('data-file')); });
          });
      ...
      function showSignModalSinSettings(file) {
        pendingFileSinSettings = file;
        document.getElementById('modalCertType').value = 'all';
        document.getElementById('signModal').classList.add('open');
      }
      ...
      function showSignModalImgText(file) {
        pendingFileImgText = file;
        document.getElementById('modalCertType').value = 'all';
        document.getElementById('signModal').classList.add('open');
      }
      ...
      document.getElementById('modalConfirm').addEventListener('click', async function() {
        ...
        } else if (pendingFileSinSettings) {
      ...
      async function doSignSinSettings(selectedFile, userToken, certificateType) {
        if (!selectedFile) { showStatus('Documento no válido.', 'error'); return; }

        const btn = document.querySelector('.btn-sign-nosettings[data-file="' + escapeAttr(selectedFile) + '"]');
        const originalHtml = btn ? btn.innerHTML : '';
        if (btn) { btn.disabled = true; btn.innerHTML = ICO_WAIT + ' Preparando...'; }
        showStatus('Obteniendo URI de firma (sin settings) para ' + selectedFile + '...', 'info');
      ...
      async function doSignImgText(selectedFile, userToken, certificateType) {
        if (!selectedFile) { showStatus('Documento no válido.', 'error'); return; }

        const GRAPHIC_URL = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTlcZ50Ci9uRJBet3r17ORbbDGEq-adGoaPS5Hm8L07qD_okGo9F6URTWE&s=10';
        const SIG_TEXT = 'Firmado digitalmente por:\n<SIGNER>\nFecha: <DATE>\nOU: <OU>\nFirmado con FirmEasy\nMotivo: {{signature_reason}}';

        const btn = document.querySelector('.btn-sign-img-text[data-file="' + escapeAttr(selectedFile) + '"]');
        const originalHtml = btn ? btn.innerHTML : '';
        if (btn) { btn.disabled = true; btn.innerHTML = ICO_WAIT + ' Preparando...'; }
        showStatus('Obteniendo URI de firma (imagen+texto) para ' + selectedFile + '...', 'info');
        ...
        showStatus('Abriendo app FirmEasy para firmar ' + selectedFile + ' (imagen+texto)...', 'info');
      ...
      async function doSignSingleCSV(fileName) {
        ...
        var btn = document.querySelector('.btn-sign-single[data-file="' + escapeAttr(fileName) + '"]');
        var originalHtml = btn ? btn.innerHTML : '';
        if (btn) { btn.disabled = true; btn.innerHTML = ICO_WAIT + ' Preparando...'; }
        showStatus('Generando CSV para ' + fileName + '...', 'info');
      ...
      async function doSignFailCSV(fileName) {
        var ICO_WAIT = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M12 6v6l4 2"></path></svg>';
        var btn = document.querySelector('.btn-sign-fail[data-file="' + escapeAttr(fileName) + '"]');
        var originalHtml = btn ? btn.innerHTML : '';
        if (btn) { btn.disabled = true; btn.innerHTML = ICO_WAIT + ' Preparando...'; }
        showStatus('Generando CSV con subida fallida para ' + fileName + '...', 'info');
      ...
      async function doDownloadFailCSV(fileName) {
        var ICO_WAIT = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M12 6v6l4 2"></path></svg>';
        var btn = document.querySelector('.btn-download-fail[data-file="' + escapeAttr(fileName) + '"]');
        var originalHtml = btn ? btn.innerHTML : '';
        if (btn) { btn.disabled = true; btn.innerHTML = ICO_WAIT + ' Preparando...'; }
        showStatus('Generando CSV con descarga fallida para ' + fileName + '...', 'info');
      ...
      async function openAppBatch() {
        ...
        try {
          ...
          showStatus('Error generando batch: ' + err.message, 'error');
        }
